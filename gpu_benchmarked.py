...
from jetsontools import Tegrastats, parse_tegrastats, filter_data, get_powerdraw
...
tegrastats_path = Path("output_stats_gpu.txt")
timestamps = []
timestamp_index = 0

tegrastats_path = Path("output_stats_gpu.txt")
with Tegrastats(tegrastats_path, interval=5):
    timestamps.append((time.time(), time.time() + 10))
    ...
    while cap.isOpened():
        ...
        if time.time() - interval_start >= 10:
            ...
            # Just use the entire output, no filtering by tag
            if not output:
                print(f"[WARNING] No tegrastats data available at frame {frame_count}. Skipping.")
            else:
                power_data = get_powerdraw(output)
                save_unified_benchmark("gpu", interval_data, tegrastats_path, (frame_count, frame_count), header_written)
                header_written = [True]
