...
def annotate_frame(frame, detections, fps):
    color = (0, 0, 255)  # Default color for text and bounding boxes
    ...
    for detection in detections:
        ...
        fall_detected = (width / height) > 1.4  # If width-to-height ratio is high, classify as fall

        # Set text and color based on fall detection
        text = "Fall Detected" if fall_detected else "Standing"
        color = (0, 0, 255) if fall_detected else (255, 255, 255)  # Red for fall, white otherwise

        # Draw bounding box
        cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), color, 2)

        # Display label
        cv2.putText(frame, LABELS[detection.label], (x_min + 10, y_min + 25), cv2.FONT_HERSHEY_TRIPLEX, 1, color)
        cv2.putText(frame, f"{int(detection.confidence * 100)}%", (x_min + 10, y_min + 60), cv2.FONT_HERSHEY_TRIPLEX, 1, color)

        # Display fall detection text above bounding box
